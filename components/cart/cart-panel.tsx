...
export function CartPanel({ isOpen, onClose, tableNumber }: CartPanelProps) {
  ...
  const subtotal = items.reduce((sum, item) => {
    const oldCustomTotal = item.customizations?.reduce((cSum, c) => cSum + c.price, 0) || 0
    const newCustomTotal = item.productCustomization
      ? (item.productCustomization.sizePrice || 0) +
        (item.productCustomization.selectedToppings?.reduce((cSum, t) => cSum + t.price, 0) || 0)
      : 0
    return sum + (item.price + oldCustomTotal + newCustomTotal) * item.quantity
  }, 0)
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-2 space-y-4 custom-scrollbar">
              {items.length === 0 ? (
                ...
                <AnimatePresence>
                  {items.map((item) => {
                    const itemKey = `${item.productId}-${JSON.stringify(
                      item.customizations || []
                    )}-${JSON.stringify(item.productCustomization || {})}`
                          ...
                          <div className="flex-1 flex flex-col pr-8">
                            ...
                            <div className="flex flex-wrap gap-2 mb-3">
                              {item.customizations?.map((c, idx) => (
                                <div
                                  key={idx}
                                  className="bg-white dark:bg-slate-700 px-3 py-1 rounded-full border border-slate-200 dark:border-slate-600 shadow-sm"
                                >
              ...
              {isCheckingOut ? (
                ...
                <div className="flex gap-3">
                  <Button
                    variant="outline"
                    onClick={onClose}
                    className="flex-1 h-14 rounded-2xl text-lg font-bold border-2 bg-transparent"
                  >
                    Back
                  </Button>
                  <Button
                    onClick={() => setIsCheckingOut(true)}
                    className="flex-1 h-14 rounded-2xl text-lg font-bold bg-slate-900 dark:bg-white dark:text-slate-900"
                  >
                    Order Now
                  </Button>
                </div>
